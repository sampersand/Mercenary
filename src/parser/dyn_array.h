...
#ifndef throw
#define throw(msg, file, line) { abort(); }
#endif
...
#define slice_decl(type) struct { size_t length; struct type* data; }

#define arr_decl(name, ...) \
	struct name##_struct { size_t cap; size_t siz; __VA_ARGS__ dat[]; };
#define arr_forward_decl(name) \
	struct name##_struct; \
	typedef struct name##_struct* name;

// #define arr_decl(...) struct { size_t cap; size_t siz; __VA_ARGS__ dat[]; }*
arr_forward_decl(arr_unit_array)
arr_decl(arr_unit_array, char)
...
#define arr_clear(name) { (name)->siz = 0; }
...
#define arr_copy(dst, src) do { \
	if ((dst)->siz != (src)->siz) throw(arr_err_size_mismatch, __FILE__, __LINE__); \
	memcpy((dst)->dat, (src)->dat, arr_data_size(src)); \
} while (0)
#define arr_shrink(arr) do { \
	arr = realloc(arr, sizeof(*arr) + arr_data_size(arr)); \
	arr->cap = arr->siz; \
} while (0)
#define arr_free(name) { free(name); name = NULL; }
...
inline static size_t ACB__(size_t idx, size_t limit, const char* file, int line) {
	if (idx >= limit) { throw(array_err_out_of_bounds, file, line); }
	return idx;
}

#ifdef arr_bounds_checking
#define arr_at(name, at) (((name)->dat[ACB__(at, (name)->siz, __FILE__, __LINE__)]))
#else
...
// typedef char[__builtin_types_compatible_p(typeof(dst->dat[0]), typeof(src->dat[0])) ? 1 : -1] type_check_test__;
...
#define arr_remove(name, at) do { \
		...
		if (deref[1] > 1) { \
			memcpy( \
				((char*)&deref[2]) + (arr_elem_size(name) * idx), \
...
inline static size_t arr_search__(arr_unit_array arr, const void* baseline, size_t elem_size) {
