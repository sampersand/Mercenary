...
typedef enum { UNOP_NEGATE, UNOP_NOT } unop_t;
...
arr_decl(stmt_array_t, stmt_t) arr_decl(block_array_t, block_t)
...
typedef struct {
  enum { DECL_FUNCTION, DECL_GLOBAL, DECL_IMPORT } kind;
  union {
    // DECL_FUNCTION
    fn_decl_t fn;
...
stmt_t mk_if(expr_t main_cond, block_t main_block,
             // length must be same of elif_conds and elif_blocks
             // expr_t[elif_len]
             expr_array_t elif_conds,
             // block_t[elif_len]
